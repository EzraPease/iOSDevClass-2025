...
var myRectangle = Rectangle(width: 150, height: 10)
    ...
    var heightInInches: Double = 0 {
        didSet {
            if heightInInches != oldValue {
                print("Value has been adjusted to \(heightInInches) properly")
            }
            else {
                print("Value not adjusted properly, let me fix that real quick")
    ...
    var heightInCentimeters: Double = 0 {
        didSet {
            if heightInCentimeters != oldValue {
                print("Value has set adjusted to \(heightInCentimeters) properly")
            }
            else {
                print("Value not adjusted properly, let me fix that real quick")
